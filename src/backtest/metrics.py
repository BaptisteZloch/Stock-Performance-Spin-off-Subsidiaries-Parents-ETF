...
def construct_report_dataframe(
    ...
    report_df.loc["R2", "Portfolio"] = r_squared(portfolio_returns, benchmark_returns)
    ...
    report_df.loc["CVaR", "Benchmark"] = conditional_value_at_risk(benchmark_returns)
    ...
    report_df.loc["Kelly criterion", "Benchmark"] = kelly_criterion(benchmark_returns)
    ...
    report_df.loc["Calmar ratio", "Benchmark"] = calmar_ratio(benchmark_returns, 252)
...
def conditional_value_at_risk(
    ...
    historic_CVaR = np.mean(
        returns[returns < value_at_risk(returns, level, method)].to_numpy()
    )
    return (
        float(historic_CVaR)
